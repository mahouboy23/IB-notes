...
note : 
00:00:00.000  
Bonjour et bienvenue dans la démonstration de mon application web IB Management, développée pour mon client, Monsieur A.
00:00:10.000  
IB Management est une plateforme permettant une gestion efficace des notes et des classes pour les différents utilisateurs, notamment les professeurs, les étudiants et le coordinateur.
00:00:25.000  
Voici les horodatages correspondant à chaque critère de réussite démontré dans cette vidéo.
00:00:32.000  
Lorsque vous accédez à l'application, vous êtes dirigé vers la page de connexion, où vous devez saisir votre nom d'utilisateur et votre mot de passe pour vous authentifier.
00:00:45.000  
Si les informations d'identification sont correctes, vous êtes redirigé vers la page d'accueil. Sinon, un message d'erreur s'affiche.
00:01:00.000  
Après une authentification réussie, si l'utilisateur a le rôle de coordinateur, il est redirigé vers la page d'administration, d'où il peut accéder à la gestion des utilisateurs.
00:01:15.000  
Sur la page de gestion des utilisateurs, le coordinateur peut ajouter, modifier et supprimer des utilisateurs via une interface intuitive.
00:01:30.000  
...
00:02:30.000  
L'ajout d'une nouvelle classe se fait via une fenêtre modale, où le coordinateur saisit les informations requises. Une fois validé, la classe est créée dans la base de données.
00:02:50.000  
...
00:04:10.000  
Sur la page de gestion des notes, le professeur peut ajouter, modifier et supprimer les notes des étudiants. Pour afficher les notes, le professeur doit d'abord sélectionner une classe, puis un étudiant.
00:04:35.000  
...
Fonctionnalité d'extensibilité 2 :  
Un autre exemple d'extensibilité pour mon application web serait l'intégration d'un système de génération de rapports personnalisables. Cette fonctionnalité permettrait aux enseignants et au coordinateur de créer des rapports détaillés sur les performances des étudiants, en sélectionnant des critères spécifiques tels que la période (trimestre ou année scolaire), la classe, la matière ou même des étudiants individuels. Les rapports seraient générés dynamiquement en utilisant les données stockées dans la base de données MySQL et seraient présentés dans un format convivial, tel que des tableaux et des graphiques. La génération de rapports serait gérée par le backend Node.js et Express, avec de nouvelles routes API pour traiter les requêtes de rapport et récupérer les données nécessaires. Les rapports générés pourraient être affichés dans l'interface utilisateur React et pourraient également être exportés dans des formats couramment utilisés, tels que PDF ou CSV, pour un partage et une analyse ultérieurs. Cette extension apporterait une valeur significative à l'application en fournissant des informations approfondies et personnalisées sur les performances des étudiants, permettant aux enseignants et au coordinateur de prendre des décisions éclairées et d'identifier les domaines nécessitant une attention particulière. 

en revenant a la page grade, durant le processus de l'ajout d'une note la valeur totale (Total value) est évaluer pour voir si il n'existe pas d'intervale de note dans la base de données si oui la note est convertit sur 7 en utilisant l'intervale, si non la note est automatiquement convertit sur 7